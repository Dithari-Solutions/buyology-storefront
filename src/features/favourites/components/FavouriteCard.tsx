...
export default function FavouriteCard({ item }: FavouriteCardProps) {
    const id = useId();
    const clipId = `favCardClip-${id.replace(/[^a-zA-Z0-9-]/g, "")}`;
    ...
    function handleAddToCart(e: React.MouseEvent) {
        ...
        dispatch(addItem({
            ...
            variant: { color: "Space Gray", storage: item.storage ?? "1TB" },
            price: item.price,
    ...
    return (
        <>
                                ...
                                <div className="w-full h-full bg-white border border-[#FBBB14] p-[10px] flex flex-col gap-[10px]">
                                    ...
                                    <div className="flex items-center justify-between gap-[6px] flex-shrink-0">
                                        ...
                                        <div className="flex items-center gap-[3px] bg-[#F6F4FF] rounded-full px-[7px] py-[3px] flex-shrink-0">
                                            ...
                                            <span className="text-[11px] font-bold text-[#402F75]">{item.rating}</span>
                                        </div>
                                    </div>
                                </div>
                            </motion.div>
            ...
            <div
                ...
                className="p-[10px] bg-white rounded-[20px] w-full border border-[#FBBB14] cursor-pointer hover:shadow-md transition-shadow"
            >
                {/* Image container */}
                <div className="relative overflow-hidden rounded-[20px] flex items-center justify-center h-[200px] mb-[12px]">
                    ...
                    >
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="#FBBB14" stroke="#FBBB14" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <div className="grid grid-cols-2 gap-[6px]">
                        {item.processor && (
                            <div className="flex items-center gap-[5px] bg-gray-50 rounded-[8px] px-[8px] py-[5px]">
                                ...
                                <span className="text-[11px] text-gray-600 font-medium truncate">{item.processor}</span>
                            </div>
                        )}
                        {item.ram && (
                            <div className="flex items-center gap-[5px] bg-gray-50 rounded-[8px] px-[8px] py-[5px]">
                                <Image src={RamIcon} alt="RAM" width={13} height={13} className="flex-shrink-0 opacity-60" />
                                <span className="text-[11px] text-gray-600 font-medium truncate">{item.ram}</span>
                            </div>
                        )}
                        {item.storage && (
                            <div className="flex items-center gap-[5px] bg-gray-50 rounded-[8px] px-[8px] py-[5px] col-span-2">
                                <Image src={StorageIcon} alt="Storage" width={13} height={13} className="flex-shrink-0 opacity-60" />
                                <span className="text-[11px] text-gray-600 font-medium truncate">{item.storage}</span>
                            </div>
                        )}
                    </div>
